...
class mod_utils_builder(build):
    def run(self):
        build.run(self)
        # Build the shared library. If this fails you may want to check your Makefile.
        ret = os.system("make -C utils")
        if ret != 0:
            raise RuntimeError("Building utils/libmod_utils.so failed (make returned %d)" % ret)

class mod_utils_installer(install):
    def run(self):
        # Run the standard install process first.
        install.run(self)
        source = os.path.join(os.getcwd(), "utils", "libmod_utils.so")
        if not os.path.isfile(source):
            self.announce("File '%s' not found. Attempting to build it..." % source, level=3)
            ret = os.system("make -C utils")
            if ret != 0:
                raise RuntimeError("Building utils/libmod_utils.so failed (make returned %d)" % ret)
            if not os.path.isfile(source):
                raise RuntimeError("After building, '%s' still not found." % source)
        
        # Compute the target directory for the shared library.
        # Normally, self.install_lib is computed from the installation layout.
        # When a staging root is used (via --root), we prepend that root.
        install_lib = self.install_lib
        if self.root:
            # Remove the leading separator if present so that os.path.join works as expected.
            install_lib = install_lib.lstrip(os.sep)
            target = os.path.join(self.root, install_lib, "modtools", "libmod_utils.so")
        else:
            target = os.path.join(self.install_lib, "modtools", "libmod_utils.so")
        
        # Ensure the target directory exists.
        os.makedirs(os.path.dirname(target), exist_ok=True)
        self.announce("Copying %s to %s" % (source, target), level=2)
...
setup(
    ...
    scripts=[],
    data_files=[
        (('share/mod/default.pedalboard'), glob('default.pedalboard/*')),
